...
@Component({
  selector: 'app-currently-tvshow',
  templateUrl: './currently-tvshow.component.html',
  styleUrls: ['./currently-tvshow.component.css']
})
export class CurrentlyTvshowComponent {

  currentlytvShows: any[] = [];
  page: number = 1
  loading = true;
  ...
  ngOnInit(): void {
    setTimeout(() => {
      this.loadCurrentlyTvShows();
      this.loading = false;
    }, 3000);
  }

  loadCurrentlyTvShows(): void {
    this.apiService.getCurrenltyTvshow('tv', this.page).subscribe((res) => {
      this.currentlytvShows = [...this.currentlytvShows, ...res.results];
      console.log(res);
